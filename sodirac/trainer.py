...
class train_integrate(object):
    ...
    def _train(
        ...
        ###### training loop
        pbar = tqdm(range(1, epochs+1), desc="DIRAC integrate..")
        for epoch in pbar:
            running_loss = 0.0
            ...
            for graph in samples["graph_dl"]:
                ...
                ######## get total loss
                loss_total =  loss_re + loss_domain + loss_MMD + loss_com
            ...
            pbar.set_postfix(
                        {"Loss": epoch_loss, "Domain": epoch_domain_loss, "RE": epoch_re_loss, 
                         "Combine": epoch_com_loss, "MMD": epoch_MMD_loss
                         } 
                        )
        torch.save({'DIRAC_weights': self.minemodel.state_dict(),
                }, os.path.join(self.save_path, "DIRAC_integration.pt"))
...
class train_annotate(train_integrate):
    ...
    def evaluate_novel_target(
        ...
        with torch.no_grad():
            ...
            for _, graph in enumerate(graph_dl):
                ...
                feat, _, _, output = self.minemodel([graph_cp], [edge_index_cp])
            ...
            outputs_numpy = all_outputs.detach().cpu().numpy()
        ...
        if return_lists_roc:
            return feats_numpy, outputs_numpy, prob_numpy, preds, confs, mean_uncert
        return preds
    ...
    def _train(
        ...
        for epoch in pbar:
            ...
            for iter_id, (graph_source, graph_target) in enumerate(zip(graph_dl_source, graph_dl_target)):
                ...
                # from https://github.com/yizhe-ang/AdaMatch-PyTorch/blob/main/trainers/adamatch.py
                self._disable_batchnorm_tracking(self.minemodel)

                _, _, _, logits_source_pp = self.minemodel([source_data], [source_edge_index])
                logits_source_pp = logits_source_pp[0]

                self._enable_batchnorm_tracking(self.minemodel)
                ...
                final_target_pseudolabels = torch.max(final_logits_target, 1)[1] 
                target_loss = self._compute_target_loss(final_target_pseudolabels,
                                                        label_preds[1],
                                                        mask,
                                                        )
    ...
    def evaluate_source(
        ...
        with torch.no_grad():
            ...
            feats, _, _, label_preds = self.minemodel([data], [edge_index])
            output = F.softmax(label_preds[0], dim = 1)
            # numpify
            feats_numpy = feats[0].detach().cpu().numpy()
    ...
    def evaluate_target(
        ...
        with torch.no_grad():
            ...
            edge_index = graph_dl["edge_index"].to(self.device)
            feats, _, _, label_preds = self.minemodel([data], [edge_index])
            prob = F.softmax(label_preds[0], dim = 1)
            conf, pred = prob.max(1)
            # numpify
            
            feats_numpy = feats[0].detach().cpu().numpy()
            output_numpy = label_preds[0].detach().cpu().numpy()
